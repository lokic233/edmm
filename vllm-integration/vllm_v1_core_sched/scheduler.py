...
class Scheduler(SchedulerInterface):
    ...
    def _is_blocked_waiting_status(status: RequestStatus) -> bool:
        return status in (
            ...
            RequestStatus.WAITING_FOR_TOOL_CALL,
        )
    ...
    def _try_promote_blocked_waiting_request(self, request: Request) -> bool:
        ...
        if request.status == RequestStatus.WAITING_FOR_TOOL_CALL:
            from vllm.v1.core.sched.edmm_hook import EDMM_ENABLED

            if EDMM_ENABLED and hasattr(self, "_edmm_hook"):
                if not self._edmm_hook.is_tool_response_ready(request.request_id):
                    return False
                self._edmm_hook.on_tool_call_resume(request.request_id)
                request.status = RequestStatus.WAITING
                return True
            return False
